...
- [x] [AI-Review-R17][LOW] `SubTaskList` passes raw `onMutate` prop to `SortableSubTaskItem` children instead of stable `triggerMutate` wrapper — defeats `React.memo` if parent passes an unstable function ref (e.g. inline arrow or un-memoised callback). Fix: `<SortableSubTaskItem … onMutate={triggerMutate} />` so all children depend on the stable safe-mutate wrapper. `src/components/SubTaskList.tsx:307`
- [x] [AI-Review-R17][LOW] `handleDragEnd` depends on raw `onMutate` in `useCallback` dep-array `[sorted, taskId, onMutate, toast]`, asymmetric with every other mutation path that goes through stable `triggerMutate`. `onMutate` is needed as `await`-able (R6 visual-flush guarantee) so `triggerMutate` (void) is not a drop-in. Backlog: introduce `awaitableTriggerMutate = useCallback(async () => { try { await onMutate(); } catch {} }, [onMutate])` and depend on that stable reference instead of raw `onMutate`. `src/components/SubTaskList.tsx:261`
- [x] [AI-Review-R17][LOW] Missing in-flight toggle double-click guard test — `toggling` state guard and `disabled={toggling}` checkbox exist but no test verifies a second rapid click is suppressed while the first `toggleSubTask` call is in-flight. Add CMP-063: spy on `apiClient.toggleSubTask` with deferred promise, fire two checkbox clicks, assert spy called exactly once. `src/test/story-3-3-subtask-edit-reorder.test.tsx`
- [x] [AI-Review-R17][LOW] `openAddDialog` in `DailyUpdateFeed` depends on `activeMembers` in `useCallback` deps, causing recreation on every `members` prop change (even when active-member list is unchanged). Inconsistent with R15 ref-optimization applied to other callbacks in the same file. Consider `activeMembersRef` pattern or snapshot `activeMembers` into a ref synced via `useEffect`. `src/components/DailyUpdateFeed.tsx:66`

### Review Follow-ups Round 18 (AI)

- [x] [AI-Review-R18][LOW] `SortableSubTaskItem` double-wraps `useSafeMutate` — R17 fixed parent to pass `triggerMutate` (already a `useSafeMutate` result) as `onMutate` to children. But `SortableSubTaskItem` still calls `useSafeMutate(onMutate)` internally, creating a nested `Promise.resolve(Promise.resolve(…))` chain per call. Remove the inner `useSafeMutate` call and use `onMutate` directly as `triggerMutate` (it is already safe). `src/components/SubTaskList.tsx:46`
- [x] [AI-Review-R18][LOW] `handleAddUpdate` and `handleEditSave` in `DailyUpdateFeed` recreate on every in-flight loading state toggle — `updateLoading` and `editUpdateLoading` in `useCallback` dep-arrays (lines 101, 127) cause 2 extra callback recreations per submission. Parity gap with `updateContentRef`/`editContentRef`/`updateAuthorRef` ref-optimizations (R14/R15). Fix: introduce `updateLoadingRef = useRef(false)` and `editUpdateLoadingRef = useRef(false)` for the guard checks; remove the state values from callback deps to make both fully stable. `src/components/DailyUpdateFeed.tsx:101,127`
- [x] [AI-Review-R18][LOW] `handleEditSave` silent empty-content exit — `if (!normalizedContent) return` (line 108) exits with zero user feedback, whereas `SubTaskList.saveEdit` shows a destructive toast for the same condition. UI-shielded by `disabled={!editingContent.trim()}` on the Save button, but the programmatic callsite asymmetry is a latent maintenance trap. Add `toast({ variant: 'destructive', title: 'Error', description: 'Update content is required' })` before the silent return. `src/components/DailyUpdateFeed.tsx:108`

### Review Follow-ups Round 19 (AI)

- [x] [AI-Review-R19][MEDIUM] `DailyUpdateFeed.handleDelete` dep array includes `deletingUpdate` + `deleteUpdateId` state — R18 parity gap. R18 introduced `updateLoadingRef`/`editUpdateLoadingRef` to stabilize `handleAddUpdate`/`handleEditSave`, but `handleDelete` (line 141) was not updated. Every `setDeletingUpdate(true/false)` and `setDeleteUpdateId()` triggers a new callback binding. Fix: introduce `deletingUpdateRef = useRef(false)` and `deleteUpdateIdRef = useRef<string | null>(null)`, sync them alongside state, and guard/access via refs inside the callback. `src/components/DailyUpdateFeed.tsx:141-154`
- [x] [AI-Review-R19][MEDIUM] `saveEdit` unconditionally closes edit mode on API failure — user loses typed content and has no retry path. After `apiClient.editSubTask` rejects, the inner catch shows a destructive toast but falls through to the outer `setEditing(false)`. Fix: move `setEditing(false)` into the inner `try` success branch only; in the catch, retain `editTitleRef.current = trimmed` to preserve the user's text so they can retry without retyping. `src/components/SubTaskList.tsx:120-133`
- [x] [AI-Review-R19][LOW] `handleDragEnd` outer `finally` triple-redundant `setReordering(false)` — normal paths already clear it (inner error-catch line 287; success path line 293) before `finally` fires again at line 298. Three state writes per drag, React batches them so no extra renders, but the logic is hard to audit. Simplify: remove the two inner `setReordering(false)` calls and rely solely on the `finally` version as the single exit point. `src/components/SubTaskList.tsx:287,293,298`
- [x] [AI-Review-R19][LOW] `SortableSubTaskItem` has misleading `const triggerMutate = onMutate` dead-rename after R18. R18 removed the inner `useSafeMutate(onMutate)` call; the alias now semantically misleads readers into thinking a hook-level transformation is occurring. Either use `onMutate` directly at the four call sites (lines ~66, 77, 97, 119) or add an explicit comment: `// parent already passes useSafeMutate result — no wrapping needed`. `src/components/SubTaskList.tsx:46`
- [x] [AI-Review-R19][LOW] No test for `editSubTask` API call failure in `saveEdit` — `CMP-018/019/020/021` cover success and validation failures; the inner `catch` for a real API rejection (network error, 403) has zero coverage. Add: spy `editSubTask` to reject, type a valid title, press Enter, assert destructive toast and edit mode state. `src/test/story-3-3-subtask-edit-reorder.test.tsx`
- [x] [AI-Review-R19][LOW] `SubTaskList` add-input Enter key path untested — CMP-002 and CMP-049 only exercise the add flow via `fireEvent.click(addButton)`. The `onKeyDown` Enter handler is a primary keyboard UX path with zero test coverage. Add a test: type input value, `fireEvent.keyDown(input, { key: 'Enter' })`, assert `apiClient.addSubTask` called. `src/test/story-3-3-subtask-edit-reorder.test.tsx`
- [x] [AI-Review-R19][LOW] `handleDragEnd` stale-ID drag guard (`oldIndex < 0 || newIndex < 0`) untested — CMP-046 (over=null) and CMP-061 (same-position) cover adjacent guards; a drag event with `active.id` not in the current sorted list exits silently via this guard with no test. Add: `capturedOnDragEnd({ active: { id: 'ghost-id' }, over: { id: 's1' } })`, assert `reorderSubTasks` not called. `src/test/story-3-3-subtask-edit-reorder.test.tsx`
...
- 2026-03-01: Code review round 18. All 10 ACs verified implemented. All R1–R17 follow-ups verified resolved. `tsc --noEmit` clean. 196/196 tests pass (12 files). 0 git discrepancies. 0 HIGH, 0 MEDIUM, 3 LOW new issues (double `useSafeMutate` wrap in `SortableSubTaskItem`; `handleAddUpdate`/`handleEditSave` callback churn per loading state toggle; `handleEditSave` silent empty-content exit vs `saveEdit` destructive toast asymmetry). 3 action items created under "Review Follow-ups Round 18 (AI)". Status → done (all items non-blocking polish).
- 2026-03-01: R18 remediation completed. Removed redundant `useSafeMutate` wrap inside `SortableSubTaskItem`, ref-guarded `DailyUpdateFeed` add/edit submit handlers with `updateLoadingRef`/`editUpdateLoadingRef`, and added explicit empty-content validation toast in `handleEditSave` (with Save button allowing the validation path to surface). Added CMP-065 and CMP-066 regression tests. `npx tsc --noEmit` clean; targeted Story 3.3 suites 63/63 passing; full `npm run test` 198/198 passing (12 files). Status → done.
- 2026-03-01: Code review round 19. All 10 ACs verified implemented. All R1–R18 follow-ups verified resolved. `tsc --noEmit` clean. 198/198 tests pass (12 files). 0 git discrepancies. 0 HIGH, 2 MEDIUM (`DailyUpdateFeed.handleDelete` dep-array R18 parity gap; `saveEdit` editor-closed-on-API-failure UX loss), 5 LOW (triple-redundant `setReordering` in `handleDragEnd` finally; misleading `triggerMutate` alias post-R18; no `editSubTask` API failure test; no add-input Enter key test; stale-ID drag guard untested). 7 action items created under "Review Follow-ups Round 19 (AI)". Status → done (all items non-blocking polish).
- 2026-03-01: R19 remediation completed. Stabilized `DailyUpdateFeed.handleDelete` via `deletingUpdateRef`/`deleteUpdateIdRef`, preserved sub-task inline edit mode on `editSubTask` API failure, simplified `handleDragEnd` to a single `setReordering(false)` exit path, and removed misleading mutation aliasing in `SortableSubTaskItem`. Added CMP-067/CMP-068/CMP-069/CMP-070 regression coverage. `npx tsc --noEmit` clean; targeted Story 3.3 suites 67/67 passing; full `npm run test` 202/202 passing (12 files). Status → done.
### Change Log
...
| 2026-03-01 | AI Code Review (R18) | Round 18 review: All 10 ACs verified implemented. All R1–R17 follow-ups verified resolved. `tsc --noEmit` clean. 196/196 tests pass (12 files). 0 git discrepancies. 0 HIGH, 0 MEDIUM, 3 LOW (`SortableSubTaskItem` inner `useSafeMutate` double-wraps already-safe `triggerMutate`; `handleAddUpdate`/`handleEditSave` recreate on loading state toggles — parity gap with R14/R15 ref-optimizations; `handleEditSave` silent empty-content exit vs `saveEdit` destructive toast asymmetry). 3 action items created under "Review Follow-ups Round 18 (AI)". Status → done (all items non-blocking polish). |
| 2026-03-01 | Dev Agent (Codex) | R18 remediation completed: removed redundant inner `useSafeMutate` in `SortableSubTaskItem`, stabilized daily-update add/edit callbacks with loading refs, added explicit empty-content edit toast path, and added CMP-065/CMP-066 regression tests. `npx tsc --noEmit` clean; targeted Story 3.3 suites 63/63 and full `npm run test` 198/198 passing. Status → done. |
| 2026-03-01 | AI Code Review (R19) | Round 19 review: All 10 ACs verified implemented. All R1–R18 follow-ups verified resolved. `tsc --noEmit` clean. 198/198 tests pass (12 files). 0 git discrepancies. 0 HIGH, 2 MEDIUM (`DailyUpdateFeed.handleDelete` dep-array R18 parity gap; `saveEdit` closes edit mode on API failure losing user's typed content), 5 LOW (triple-redundant `setReordering` in `handleDragEnd` finally; misleading `triggerMutate` alias in `SortableSubTaskItem`; no `editSubTask` API failure test; no add-input Enter key test; stale-ID drag guard untested). 7 action items created under "Review Follow-ups Round 19 (AI)". Status → done (all items non-blocking polish). |
| 2026-03-01 | Dev Agent (Codex) | R19 remediation completed: stabilized delete callback guards via refs, preserved sub-task edit mode on API failure, simplified drag-end reordering cleanup, removed misleading mutate aliasing, and added CMP-067/CMP-068/CMP-069/CMP-070 coverage. `npx tsc --noEmit` clean; targeted Story 3.3 suites 67/67 and full `npm run test` 202/202 passing. Status → done. |
### File List
...
| `src/test/story-3-3-subtask-edit-reorder.test.tsx` | Created | 39 tests for sub-task inline edit + drag reorder/delete |
| `src/test/story-3-3-daily-update-gating.test.tsx` | Created | 28 tests for daily update 24h gating + error toasts |
