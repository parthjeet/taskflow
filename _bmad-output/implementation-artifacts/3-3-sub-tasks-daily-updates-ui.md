...
- [ ] [AI-Review-R13][MEDIUM] `DailyUpdateFeed.handleEditSave` missing same-content no-op guard — editing an update without changing the text calls `apiClient.editDailyUpdate`, sets `edited: true` (adding spurious `(edited)` badge), bumps `updatedAt`, and fires parent refresh. Parity gap with `SubTaskList.saveEdit` which short-circuits on `trimmed === sub.title`. Add `if (normalizedContent === currentUpdateContent.trim()) { setEditingUpdateId(null); return; }` guard by threading the original content into the handler (e.g. capturing `upd.content` at click time in a ref). `src/components/DailyUpdateFeed.tsx:85-89`
- [ ] [AI-Review-R13][LOW] `deleteUpdateId` not reset on `taskId` change — `useEffect(() => { setEditingUpdateId(null); setEditingContent(''); }, [taskId])` omits `deleteUpdateId`/`deletingUpdate`. If the delete AlertDialog is open when `taskId` changes, confirming calls `deleteDailyUpdate(newTaskId, oldUpdateId)` → surfaces a 404 error toast while the stale dialog remains unexpectedly open. Fix: add `setDeleteUpdateId(null); setDeletingUpdate(false);` to the same `useEffect`. `src/components/DailyUpdateFeed.tsx:56-58`
- [ ] [AI-Review-R13][LOW] `SortableSubTaskItem` title span missing accessible edit-action hint — `role="button"` + `tabIndex={0}` allows keyboard focus but screen readers announce only the sub-task text with no indication that Enter activates inline edit mode. Add `aria-label={\`Edit sub-task: ${sub.title}\`}` to give keyboard/AT users a clear action label. Add a companion a11y assertion in `story-3-3-subtask-edit-reorder.test.tsx`. `src/components/SubTaskList.tsx:168`
- [ ] [AI-Review-R13][LOW] `reorderSubTasks` mock mutates sub-task objects in-place — `subTaskIds.map((id, idx) => { const sub = task.subTasks.find(...); sub.position = idx; return sub; })` modifies the live `task.subTasks[n]` reference directly before the spread copy. Diverges from `toggleSubTask`/`editSubTask` which build the mutation then return `{ ...sub }` defensively. Replace with `const updated = { ...sub, position: index }` and rebuild the array from copies. `src/lib/api/adapters/mock.ts:406-412`
- [ ] [AI-Review-R13][LOW] No test for "No active members" disabled state — `openAddDialog` sets `updateAuthor = ''` when `activeMembers` is empty; the Select renders a disabled `"No active members"` placeholder and the Add button is disabled. No test exercises this branch. Add a test passing `members` where all have `active: false` and verify Add button is disabled and the placeholder text is present. `src/test/story-3-3-daily-update-gating.test.tsx`
...
| `src/test/story-3-3-daily-update-gating.test.tsx` | Created | 20 tests for daily update 24h gating + error toasts |
