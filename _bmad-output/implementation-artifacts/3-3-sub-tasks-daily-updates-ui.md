...
- [ ] [AI-Review-R12][LOW] `CMP-041` (Escape cancels without API call) is vacuous in jsdom — jsdom does not fire native `blur` on DOM removal, so the production Escape→blur bug (M1 above) was invisible in tests before this round. CMP-054 now covers the real browser scenario. `src/test/story-3-3-subtask-edit-reorder.test.tsx`
- [ ] [AI-Review-R12][LOW] `handleEditKeyDown` and `handleEditBlur` callbacks in `SortableSubTaskItem` are recreated on every keystroke because `saveEdit` depends on `editTitle` state. Three function allocations per character typed. Negligible at ≤20 items but avoidable by using a mutable `editTitleRef` for the in-progress title value (read inside `saveEdit`, updated via `onChange` on the ref) while keeping display state for controlled input. `src/components/SubTaskList.tsx`
...
| `src/test/story-3-3-subtask-edit-reorder.test.tsx` | Created | 31 tests for sub-task inline edit + drag reorder/delete |
| `src/test/story-3-3-daily-update-gating.test.tsx` | Created | 19 tests for daily update 24h gating + error toasts |
